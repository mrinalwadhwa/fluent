...
The concrete discipline the architecture reviewer checks: **release the queue lock before mutating the Work model.** For example, the scheduler drops the queue lock before `ensure_bound_attempt` mutates the Work model, so the queue lock is never held while a Work/lineage/candidate/follow-up lock is taken.
