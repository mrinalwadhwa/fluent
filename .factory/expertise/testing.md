...
**Behavioral tests** (`tests/behaviors/`) — verify the factory delivers its specified behaviors. Written from EARS statements without seeing code. Test the system from the outside.
...
Set `FACTORY_TESTS_SKIP_LOG=1` to disable per-case log writing (useful for CI workers that manage their own artifact capture).
