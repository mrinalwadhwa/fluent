...
fn work_attempt_run_drives_write_reviews_and_passes() {
    ...
    factory_cmd()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Completed Task attempt-1-write"))
        .stdout(predicate::str::contains(
            "Planned 5 review Tasks for Attempt attempt-1",
        ))
        .stdout(predicate::str::contains(
...
"##,
    ...
    assert!(
        candidate["merge_state"]["failure_reason"]
            .as_str()
            .unwrap()
            .contains("Merge-time reviewer documentation dirtied candidate workspace")
    );
    assert!(
        candidate["merge_state"]["review_artifacts"]
            .as_array()
            .unwrap()
            .iter()
            .any(|artifact| artifact["producer_id"] == "merge-review-documentation")
    );
}
...
fn cleanup_work_items_dry_run_and_apply_manage_state_worktree_and_branch() {
    ...
    let branch_name = "work/work-1/attempt-1/attempt-1-write";
    StdCommand::new("git")
        .args([
            "worktree",
            "add",
