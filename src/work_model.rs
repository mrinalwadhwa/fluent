...
mod tests {
    ...
    #[test]
    fn reviewer_prompts_mention_tester_results_authoritative() {
        let prompts = [
        ...
        for path in &prompts {
            ...
            assert!(
                content.contains("tester-results.json") && content.contains("authoritative"),
                "{path} should mention tester-results.json as authoritative"
            );
        }
    }

    #[test]
    fn reviewer_prompts_disallow_canonical_rerun() {
        let prompts = [
            "prompts/review-architecture.md",
            "prompts/review-behaviors.md",
            "prompts/review-documentation.md",
        ...
        for path in &prompts {
            ...
            assert!(
                normalized.contains("Do NOT re-run the canonical test suite"),
                "{path} should disallow re-running the canonical test suite"
            );
        }
    }

    #[test]
    fn reviewer_prompts_allow_adhoc_verification() {
        let prompts = [
            "prompts/review-architecture.md",
            "prompts/review-behaviors.md",
            "prompts/review-documentation.md",
        ...
        for path in &prompts {
            let content = std::fs::read_to_string(path).unwrap();
            assert!(
                content.contains("Ad-hoc verifications"),
                "{path} should allow ad-hoc verifications"
            );
        }
    }
    ...
    #[test]
    fn review_behaviors_prompt_describes_failure_interpretation() {
        let content = std::fs::read_to_string("prompts/review-behaviors.md").unwrap();
        assert!(
            content.contains("Real") || content.contains("real"),
            "review-behaviors.md should mention real failures"
        );
        assert!(
            content.contains("Infrastructure") || content.contains("infrastructure"),
    ...
    #[test]
    fn review_behaviors_prompt_handles_error_field() {
        let content = std::fs::read_to_string("prompts/review-behaviors.md").unwrap();
        assert!(
            content.contains("error") && content.contains("fail") && content.contains("kind"),
            "review-behaviors.md should instruct the reviewer to handle error field"
        );
    }
